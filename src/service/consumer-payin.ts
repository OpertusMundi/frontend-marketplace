...
export default class ConsumerPayInApi extends Api {
   ...
   * @returns
   */
  public async getPayIn(payInKey: string): Promise<ServerResponse<ConsumerBankwirePayIn | ConsumerCardDirectPayIn>> {
    const url = `/action/consumer/payins/${payInKey}`;

    return this.get<ServerResponse<ConsumerBankwirePayIn | ConsumerCardDirectPayIn>>(url)
      .then((response: AxiosServerResponse<ConsumerBankwirePayIn | ConsumerCardDirectPayIn>) => {
        const { data } = response;
  ...
  ): Promise<AxiosPageResponse<ConsumerBankwirePayIn | ConsumerCardDirectPayIn>> {
    const { id: field, order } = sorting;

    const url = `/action/consumer/payins?page=${page}&size=${size}&status=${status || ''}&orderBy=${field}&order=${order}`;

    return this.get<ServerResponse<PageResult<ConsumerBankwirePayIn | ConsumerCardDirectPayIn>>>(url);
  }
}
