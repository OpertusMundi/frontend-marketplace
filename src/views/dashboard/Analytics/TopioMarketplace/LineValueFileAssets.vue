...
export default class LineValueFileAssets extends Vue {
  ...
  getAnalytics(): void {
    ...
    console.log(query);
    this.analyticsApi.executeTotalAssetValueQuery(query)
      .then((response) => {
        console.log('executeTotalAssetValueQuery: ', response);
        if (response.success) {
          ...
          this.seriesData = [{ name: 'Asset Total Value', data: dataSeries }];
  ...
  async getAssets(): Promise<any> {
    ...
    this.draftAssetApi.find(query, pageRequest, sort)
      .then((resp) => {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (a) => {
          ...
          return this.symbol === ''
            ? `${point.y} <br>${point.category}<br>${point.series.name}`
            : `${point.y
              .toString()
              .replace(/(\.\d{2})\d*/, '$1')
