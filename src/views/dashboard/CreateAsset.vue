          ...
          <transition name="fade" mode="out-in">
            <div class="dashboard__form__steps__inner" :style="currentStep === 2 && assetMainType === 'COLLECTION' ? 'height: auto' : ''">
              ...
              <template v-if="assetMainType === 'API'">
                ...
                <contract ref="step5" :contractTemplateType.sync="asset.contractTemplateType" :contractTemplateKey.sync="asset.contractTemplateKey" :customContractToUpload.sync="customContractToUpload" :assetMainType="assetMainType" v-if="currentStep === 5"></contract>
                <payout ref="step6" :selectedPayoutMethod.sync="selectedPayoutMethod" v-if="currentStep === 6"></payout>
                <review ref="step7" :apiCreationType="apiCreationType" :apiSourceAsset="apiCreationType === 'PUBLISHED_ASSET' ? selectedPublishedAssetForApiCreation : apiCreationType === 'TOPIO_DRIVE' ? asset : null" :accessToFileType="getAccessToFileType" :vettingRequired.sync="asset.vettingRequired" :iprProtectionEnabled.sync="asset.iprProtectionEnabled" :errors="errors" :asset="{ ...selectedPublishedAssetForApiCreation, ...{ contractTemplateKey: asset.contractTemplateKey, pricingModels: asset.pricingModels, spatialDataServiceType: asset.spatialDataServiceType } }" v-if="currentStep == 7" @goToStep="goToStep"></review>
              </template>
