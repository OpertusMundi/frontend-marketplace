  ...
  <div class="page" v-if="page.length > 0">
    ...
    <div class="m_container">
      <div class="faq__category">
        <div class="faq__category__inner" v-for="(item, index) in category" v-bind:key="`${index}_category`">
          <div class="faq__category__title">
            {{ item.name }}
          </div>
          <router-link :to="`${page[0].slug}/${post.post_name}`" class="faq__category__item" v-for="(post, index) in item.posts" v-bind:key="`${index}_post`">
            {{ post.post_title }}
          </router-link>
        </div>
      </div>
...
export default class FaqCategory extends Vue {
  ...
  mounted(): void {
    axios
      .get(`${this.wpUrl}/wp-json/wp/v2/faq_categories?slug=${this.$route.params.category}`)
      .then((response) => {
    ...
    axios
      .get(`${this.wpUrl}/wp-json/om/v1/faq-categories?category=${this.$route.params.category}`)
      .then((response) => {
